...
router.get('/', async (req, res) => {
  try {
    const s = await db.staff.findMany({ where:{ storeId:req.staff.storeId }, select:{ id:true, name:true, role:true, type:true, wage:true, isAdmin:true, fixedOvertimeHours:true, standardMonthlyHours:true }, orderBy:{ name:'asc' } })
    res.json(s)
...
router.post('/', admin, async (req, res) => {
  try {
    ...
    const s = await db.staff.create({ data:{ storeId:req.staff.storeId, name, role:role||'スタッフ', type:type||'part-time', wage:wage||1000, pin: await bcrypt.hash(pin,10), isAdmin:isAdmin||false, fixedOvertimeHours:fixedOvertimeHours||20, standardMonthlyHours:standardMonthlyHours||160 } })
