...
sed -i '' "s/wishes: Object.values(wishes).filter((w: any) => w.status !== 'none')/wishes: Object.values(wishes)/" ~/Downloads/barshift-v2/src/pages/Wish.tsx && echo "✅ Wish.tsx修正完了"
cat > ~/Downloads/barshift-v2/server/routes/wishes.js << 'ENDOFFILE'
import { Router } from 'express'
import { PrismaClient } from '@prisma/client'
import { admin } from '../middleware/auth.js'
const router = Router(), db = new PrismaClient()
