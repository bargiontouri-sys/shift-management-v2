...
export default function Wish() {
  ...
  const updT = (k:string, f:'start'|'end', v:string) => setWishes(p => ({ ...p, [k]: { ...p[k], [f]:v } }))
  const submit = async () => {
    setSaving(true)
    try { await api.post('/api/wishes/bulk', { wishes: Object.values(wishes).filter((w:any) => w.status!=='none') }); toast.success('希望を提出しました！') }
    ...
    <div className="fu" style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      <div className="card" style={{ padding:14 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:3 }}>
          {cells.map((d,i)=>{
            ...
            const k=key(d), st=wishes[k]?.status||'none', hw=HOLIDAYS[k], dw=new Date(y,m,d).getDay()
            const isClosed=CLOSED_DAYS.includes(dw)
            const bg=isClosed?'var(--bg)':st==='ok'?'rgba(0,232,122,0.12)':st==='ng'?'rgba(255,61,90,0.12)':'var(--bg3)'
            const bc=isClosed?'var(--bd)':st==='ok'?'rgba(0,232,122,0.4)':st==='ng'?'rgba(255,61,90,0.4)':'var(--bd)'
            const tc=isClosed?'var(--tx3)':st!=='none'?(st==='ok'?'var(--green)':'var(--red)'):dw===0||hw?'var(--red)':dw===6?'var(--blue)':'var(--tx3)'
            return (
              <button key={k} onClick={()=>toggle(k)}
                style={{ aspectRatio:'1', background:bg, border:`1.5px solid ${bc}`, borderRadius:'var(--r)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', cursor:isClosed?'not-allowed':'pointer', transition:'all 0.12s', gap:1, opacity:isClosed?0.4:1 }}>
                <span style={{ fontSize:10, fontWeight:900, color:tc, fontFamily:'var(--mono)' }}>{d}</span>
                {isClosed?<span style={{ fontSize:7, color:'var(--tx3)' }}>休</span>:st==='ok'?<CheckCircle2 size={10} color="var(--green)"/>:st==='ng'?<XCircle size={10} color="var(--red)"/>:<div style={{ height:10 }}/>}
              </button>
            )
          })}
        </div>
      ...
      {okDays.length>0&&(
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          <p style={{ fontSize:9, color:'var(--tx3)', fontWeight:900, textTransform:'uppercase', letterSpacing:'0.15em' }}>出勤希望時間</p>
      ...
      <button className="btn btn-p" onClick={submit} disabled={saving||okDays.length===0} style={{ width:'100%' }}>
