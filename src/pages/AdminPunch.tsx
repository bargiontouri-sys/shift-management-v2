...
export default function AdminPunch() {
  ...
  const today = todayDk()
  const minDate = new Date(); minDate.setMonth(minDate.getMonth() - 6)
  ...
  return (
    <div className="fu" style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      {selStaff && (<>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <span style={{ fontSize:12, fontWeight:700 }}>{y}年{m+1}月</span>
        ...
        <div className="card">
          ...
          {addMode && (
            <div style={{ background:'var(--bg3)', border:'1px solid var(--bd2)', borderRadius:'var(--r)', padding:12, marginBottom:12 }}>
              <p style={{ fontSize:9, color:'var(--ac)', fontWeight:900, textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:10 }}>打刻を追加</p>
              <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                <select value={newType} onChange={e => setNewType(e.target.value as 'in'|'out')}
                  style={{ background:'var(--bg2)', border:'1px solid var(--bd2)', padding:'8px 12px', fontSize:12, color:'var(--tx)', outline:'none', borderRadius:'var(--r)', fontWeight:700 }}>
                  <option value="in">出勤</option>
                  <option value="out">退勤</option>
                </select>
                <input type="time" value={newTime} onChange={e => setNewTime(e.target.value)}
                  style={{ background:'var(--bg2)', border:'1px solid var(--bd2)', padding:'8px 12px', fontSize:13, color:'var(--tx)', fontFamily:'var(--mono)', outline:'none', borderRadius:'var(--r)', flex:1 }}/>
                <button onClick={addPunch}
                  style={{ padding:'8px 18px', background:'var(--ac)', border:'none', color:'#000', borderRadius:'var(--r)', fontSize:11, fontWeight:900, cursor:'pointer' }}>追加</button>
                <button onClick={() => setAddMode(false)}
                  style={{ padding:'8px 14px', background:'var(--bg4)', border:'1px solid var(--bd)', color:'var(--tx3)', borderRadius:'var(--r)', fontSize:11, cursor:'pointer' }}>×</button>
              </div>
            </div>
          )}
                  ...
                  {editPunch?.id === p.id ? (
                    <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', background:'var(--bg3)', border:'1px solid var(--bd2)', borderRadius:'var(--r)', padding:10 }}>
                      <select value={editType} onChange={e => setEditType(e.target.value as 'in'|'out')}
                        style={{ background:'var(--bg2)', border:'1px solid var(--bd2)', padding:'6px 10px', fontSize:12, color:'var(--tx)', outline:'none', borderRadius:'var(--r)', fontWeight:700 }}>
                        <option value="in">出勤</option>
                        <option value="out">退勤</option>
                      </select>
                      <input type="time" value={editTime} onChange={e => setEditTime(e.target.value)}
                        style={{ background:'var(--bg2)', border:'1px solid var(--bd2)', padding:'6px 10px', fontSize:13, color:'var(--tx)', fontFamily:'var(--mono)', outline:'none', borderRadius:'var(--r)', flex:1 }}/>
                      <button onClick={saveEdit} style={{ padding:'6px 14px', background:'var(--green)', border:'none', color:'#000', borderRadius:'var(--r)', fontSize:11, fontWeight:900, cursor:'pointer' }}>保存</button>
                      <button onClick={() => setEditPunch(null)} style={{ padding:'6px 12px', background:'var(--bg4)', border:'1px solid var(--bd)', color:'var(--tx3)', borderRadius:'var(--r)', fontSize:11, cursor:'pointer' }}>×</button>
                      <button onClick={() => deletePunch(p.id)} style={{ padding:'6px 12px', background:'rgba(255,61,90,0.1)', border:'1px solid rgba(255,61,90,0.25)', color:'var(--red)', borderRadius:'var(--r)', fontSize:11, cursor:'pointer', display:'flex', alignItems:'center', gap:4 }}><Trash2 size={12}/>削除</button>
                    </div>
                  ) : (
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', background:'var(--bg3)', border:`1px solid ${p.type==='in'?'rgba(0,232,122,0.2)':'rgba(255,61,90,0.2)'}`, borderRadius:'var(--r)', padding:'12px 14px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                        <span style={{ fontSize:10, fontWeight:900, padding:'4px 12px', borderRadius:'3px', background:p.type==='in'?'rgba(0,232,122,0.1)':'rgba(255,61,90,0.1)', color:p.type==='in'?'var(--green)':'var(--red)', border:`1px solid ${p.type==='in'?'rgba(0,232,122,0.25)':'rgba(255,61,90,0.25)'}` }}>{p.type==='in'?'出勤':'退勤'}</span>
                        <span style={{ fontFamily:'var(--mono)', fontSize:20, fontWeight:700, color:'var(--tx)' }}>{p.time}</span>
                      </div>
                      <button onClick={() => { setEditPunch(p); setEditTime(p.time); setEditType(p.type) }}
                        style={{ display:'flex', alignItems:'center', gap:5, padding:'7px 14px', background:'var(--bg4)', border:'1px solid var(--bd)', color:'var(--tx2)', borderRadius:'var(--r)', fontSize:11, fontWeight:700, cursor:'pointer' }}>
                        <Edit2 size={12}/> 修正
                      </button>
                    </div>
                  )}
