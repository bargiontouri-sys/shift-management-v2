              ...
              : storeStaff.map(s => {
                ...
                return (
                  <button key={s.id} onClick={()=>{ setSelId(s.id); setPin(''); setError('') }}
                    style={{ padding:'8px 10px', border:`2px solid ${sel?'var(--tx)':'var(--bd)'}`, background:sel?'var(--tx)':'var(--bg3)', color:sel?'var(--bg)':'var(--tx2)', borderRadius:'var(--r)', display:'flex', alignItems:'center', gap:7, fontSize:11, fontWeight:700, textAlign:'left', transition:'all 0.12s' }}>
          ...
          {selId && (
            <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              <div style={{ display:'flex', justifyContent:'center', gap:12 }}>
                {[0,1,2,3].map(i=>(
                  <div key={i} style={{ width:10, height:10, background:pin.length>i?'var(--tx)':'transparent', border:`1px solid ${pin.length>i?'var(--tx)':'var(--bd2)'}`, borderRadius:'50%', transition:'all 0.1s' }}/>
                ))}
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6, maxWidth:200, margin:'0 auto', width:'100%' }}>
                {[1,2,3,4,5,6,7,8,9,'C',0,'OK'].map(k=>(
                  <button key={k} onClick={()=>press(k)} disabled={loading}
                    style={{ aspectRatio:'1', background:k==='OK'?'var(--tx)':k==='C'?'rgba(255,61,90,0.1)':'var(--bg3)', color:k==='OK'?'var(--bg)':k==='C'?'var(--red)':'var(--tx)', border:`1px solid ${k==='OK'?'var(--tx)':k==='C'?'rgba(255,61,90,0.3)':'var(--bd)'}`, borderRadius:'var(--r)', fontSize:k==='OK'?10:15, fontWeight:900, fontFamily:'var(--mono)', transition:'opacity 0.1s' }}>
                    {k}
                  </button>
                ))}
              </div>
              {error && <p style={{ color:'var(--red)', fontSize:10, fontWeight:700, textAlign:'center', display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}><AlertCircle size={12}/>{error}</p>}
            </div>
          )}
        </div>
